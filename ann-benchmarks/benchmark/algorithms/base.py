...
class BaseANN(object):
    ...
    def get_build_memory_usage(self):
        """Return the current memory usage of this algorithm instance
        (in kilobytes), or None if this information is not available."""
        # return in kB for backwards compatibility
        time.sleep(5)
        cur_memory_usage = self.get_memory_usage()
        if float(cur_memory_usage) > self.build_memory_usage:
            self.build_memory_usage = float(cur_memory_usage)
        return self.build_memory_usage
