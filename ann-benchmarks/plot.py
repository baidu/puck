...
def create_plot(all_data, raw, x_scale, y_scale, xn, yn, fn_out, linestyles):
    ...
    for algo in sorted(all_data.keys(), key=mean_y):
        ...
        para = np.polyfit(xs, ys, 3)
        fun_y = para[0] * check_recall ** 3 + para[1] * check_recall ** 2 + para[2] * check_recall  + para[3]
        print(algo, fun_y)
        fy_list.append(fun_y)
    ...
    #ax.vlines([check_recall], 0, 30000, linestyles='dashed', colors='gray')
    #for fy in fy_list:
    #    ax.hlines([fy], 0.75, 1.0, linestyles='dashed', colors='red')
